...
Onto the modelling part. We will test several models, simple and robust ones, however, all will be done utilizing 5 repeats of 5-fold cross validation. Defined as follows using the *caret* package:
...
Utilizing prevalence of anemia in pregnancy to predict ART index, we obtain an Rsquared of ~0.68 and a relatively low MAE of ~0.099. It's a very simple model, but we can say we're off to a good start. 
...
knitr::kable(
    ...
    booktabs=TRUE,
    longtable=TRUE,
    caption="Prevalence of anemia in pregnancy ~ ART index"
)
```
...
We obtain an Rsquared of ~0.78 and a MAE of ~0.079, which is in itself an acceptable model. Perhaps this would be one of those variables that contributes to more robust and multivariate models.
...
And we find that yes!, it's actually quite good, with an Rsquared of ~0.87-0.88 and MAE of ~0.06. This variable is probably going to contribute somewhat well in more robust and multiple models.
